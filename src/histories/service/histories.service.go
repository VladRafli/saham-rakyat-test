...
func GetAll(c echo.Context) (*[]database.Histories, *echo.HTTPError) {
	...
	if takeQuery != "" && skipQuery != "" {
		...
		db.Limit(take).Preload(clause.Associations).Find(histories)
	} else if skipQuery != "" {
		db.Offset(skip).Preload(clause.Associations).Find(histories)
	} else {
		db.Preload(clause.Associations).Find(histories)
	}
